...
ROOTS=("$HOME/Library/Application Support/Google/Chrome" "$HOME/Library/Application Support/BraveSoftware/Brave-Browser" "$HOME/Library/Application Support/Microsoft Edge")
echo "🔍 Scanning..."
for r in "${ROOTS[@]}"; do
    [ -d "$r" ] && find "$r" -type d -name "Extensions" 2>/dev/null | while read ed; do
        for ep in "$ed"/*; do
            [ -d "$ep" ] && { id=$(basename "$ep"); man=$(find "$ep" -name "manifest.json" -maxdepth 2 | head -n 1); [ -f "$man" ] && echo "$id # $(jq -r '.name' "$man")" >> "$TEMP"; }
        done
    done
done
